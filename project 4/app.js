...
//選擇select內部的OPTION之後, 要改變相對應的顏色
let allSelects = document.querySelectorAll("select");
allSelects.forEach((select) => {
  select.addEventListener("change", (e) => {
    setGPA();
    changeColor(e.target); //<select>就是e.target
  });
});
...
addButton.addEventListener("click", () => {
  ...
  //製作五個小元素
  let newInput1 = document.createElement("input"); //class catagory
  newInput1.setAttribute("type", "text"); //設定屬性
  newInput1.setAttribute("list", "opt");
  newInput1.setAttribute("placeholder", "class catagory");
  newInput1.classList.add("class-type");

  let newInput2 = document.createElement("input"); //class number
  newInput2.setAttribute("type", "text"); //設定屬性
  newInput2.setAttribute("placeholder", "class number");
  newInput2.classList.add("class-number");

  let newInput3 = document.createElement("input"); //credits
  newInput3.setAttribute("type", "number"); //設定屬性
  ...
  newForm.style.animation = " scaleUp 0.5s ease forwards"; //新的表格出現時大小由0變為1
});
//垃圾桶
...
function handleSorting(direction) {
  ...
  // SELECT可直接用JS更改
  graders = document.querySelectorAll("div.grader"); //選取的是靜態的nodelist,不像innerHTML會自動更新
  for (let i = 0; i < graders.length; i++) {
